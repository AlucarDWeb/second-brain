...
| `Concepts/`  | **Atomic concept articles** — one idea per file. The heart of the wiki. Linked to each other and to Sources. | You |
...
- **Merge, don't duplicate** — before creating a concept, search `Concepts/` for an existing note (including aliases). Extend the existing one and add the new source rather than making a near-duplicate. Surface suspected duplicates during `/lint`.
